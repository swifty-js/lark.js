...
function evalChartCode(
  ...
  try {
    ...
    return { options: null, error: e instanceof Error ? e.message : String(e) };
  }
}
...
export default function EditorPage() {
  ...
  // Non-reactive refs for third-party instances (larky rule: never in deep signals)
  ...
  useEffect(() => {
    ...
    projectId.current = params.get("projectId") || "";
    chartId.current = params.get("chartId") || "";
    ...
    if (chartId.current && projectId.current) {
      getProjectDetailApi(Number(projectId.current)).then((res) => {
  ...
  const saveChart = () => {
    if (!projectId.current || saving.value) return;
    saving.value = true;
    status.value = "idle";
    saveChartApi({
      projectId: Number(projectId.current),
  ...
  return (
    <div class="bg-surface flex min-h-0 flex-1 flex-col">
      ...
      <header class="bg-surface/75 border-border flex h-12 shrink-0 items-center justify-between border-b px-4 backdrop-blur-xl backdrop-saturate-150">
        <div class="flex items-center gap-3">
          <button
            ...
            onClick={() =>
              nav(projectId.current ? `/projects?projectId=${projectId.current}` : "/projects")
            }
          >
          ...
          <input
            ...
            onInput={(e) => (chartName.value = (e.target as HTMLInputElement).value)}
          />
          {status.value === "saved" && (
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            class="bg-brand hover:bg-brand-hover hover:shadow-glow rounded-md px-3 py-1 text-xs font-medium text-white shadow-sm transition-all disabled:opacity-50"
      ...
      <div class="relative flex min-h-0 flex-1 overflow-hidden">
              ...
              {(["visual", "code", "data"] as const).map((tab) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    activeTab.value = tab;
                    if (tab === "code") queueMicrotask(ensureMonaco);
                  }}
                >
                  {tab.charAt(0).toUpperCase() + tab.slice(1)}
                  {activeTab.value === tab && (
                    <span class="bg-brand absolute inset-x-4 bottom-0 h-0.5 rounded-full"></span>
                  )}
                </button>
              ))}
            </div>
        ...
        {/* Left panel toggle */}
        <button
          class={`border-border bg-surface text-text-tertiary hover:text-brand ${leftExpand.value ? "left-[50vw]" : "left-0"} absolute top-1/2 z-10 flex h-10 w-5 -translate-y-1/2 items-center justify-center rounded-r-lg border border-l-0 shadow-sm transition-colors`}
          title="Toggle panel"
          onClick={() => {
        ...
        >
          {raw(icon(leftExpand.value ? "chevronLeft" : "chevronRight", 10))}
        </button>
            ...
            >
              {previewError.value ? (
                <div class="flex h-full flex-col items-center justify-center gap-2 p-6">
                  <span class="text-danger/50 inline-flex">{raw(icon("alertCircle", 28))}</span>
                  <p class="text-danger max-w-md text-center text-xs leading-relaxed">
                    {previewError.value}
                  </p>
                </div>
              ) : (
                <div ref={previewHost} class="h-full w-full"></div>
              )}
            </div>
          </div>
        </div>
