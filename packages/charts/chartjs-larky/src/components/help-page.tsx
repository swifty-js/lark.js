...
const SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "The platform is built with larky (routing + UI), TailwindCSS, Monaco Editor, chart.js, and GSAP animations. It supports dark mode and responsive layouts.",
    ],
  },
];
...
export default function HelpPage() {
  ...
  const activeKey = useSignal(
    SECTIONS.find(
      (s) => s.key === (new URLSearchParams(window.location.search).get("section") || "start"),
    )?.key || SECTIONS[0].key,
  );

  useEffect(() => {
    if (root.current) animateIn(root.current, "[data-anim]", { y: 14, stagger: 0.05 });
  });

  const selectSection = (key: string) => {
    activeKey.value = key;
    router.navigate(`/help?section=${key}`);
  };

  const current = SECTIONS.find((s) => s.key === activeKey.value) || SECTIONS[0];

  return (
    <div ref={root} class="mx-auto max-w-5xl px-6 py-10">
      <div class="mb-8">
        ...
        <aside class="w-48 shrink-0">
          <nav class="sticky top-20 space-y-1">
            {SECTIONS.map((item) => (
              ...
              >
                <span class="inline-flex shrink-0 opacity-70">
                  {raw(icon(item.icon as IconName, 14))}
                </span>
                {item.title}
              </button>
            ))}
          ...
          {current.code && (
            <div class="border-border animate-scale-in mt-6 overflow-hidden rounded-xl border">
              <div class="border-border bg-surface-alt flex items-center gap-2 border-b px-4 py-2">
                <span class="text-brand/70 inline-flex">{raw(icon("code", 13))}</span>
