...
export default function ProjectsPage() {
  ...
  const loggedIn = useSignal(useAuthStore.getState().loggedIn);
  ...
  const loadCharts = (projectId: number) => {
    getProjectDetailApi(projectId).then((res) => {
      if (res.ok && res.data) {
        ...
        queueMicrotask(() => {
          if (root.current) animateIn(root.current, "[data-anim-card]", { y: 20, stagger: 0.04 });
        });
      }
    });
  };
  ...
  const selectProject = (project: ProjectSummary) => {
    ...
    charts.value = [];
    nav(`/projects?projectId=${project.id}`);
  ...
  useSignalEffect(() => {
    const { loggedIn: li } = useAuthStore.getState();
    loggedIn.value = li;
    if (li) loadProjects();
  });
    ...
    return (
      <div class="mx-auto max-w-7xl px-6 py-8">
        <div class="border-border flex h-72 flex-col items-center justify-center rounded-2xl border border-dashed">
          <span class="text-text-tertiary/40 inline-flex">{raw(icon("folder", 40))}</span>
  ...
  const renderChartCard = (item: ChartItem) => (
    <div
      data-anim-card
  ...
  return (
    <div ref={root} class="mx-auto max-w-7xl px-6 py-8">
            ...
            <>
              <div class="mb-6 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => nav(`/editor?projectId=${current.value!.id}&mode=develop`)}
                >
              ...
              {charts.value.length > 0 ? (
                <div class="grid grid-cols-2 gap-4 lg:grid-cols-3">
                  {charts.value.map((item) => renderChartCard(item))}
                </div>
              ) : (
          ...
          >
            <div class="p-6">
              ...
              <input
                ...
                onInput={(e) => (dialogName.value = (e.target as HTMLInputElement).value)}
                onKeyDown={(e) => {
              ...
              <div class="mt-5 flex justify-end gap-2">
                ...
                <button
                  class="bg-brand hover:bg-brand-hover hover:shadow-glow rounded-md px-4 py-2 text-sm font-medium text-white transition-all disabled:opacity-50"
          ...
          >
            <div class="p-6">
              ...
              <select
                ...
                onChange={(e) => (cloneTargetId.value = (e.target as HTMLSelectElement).value)}
              >
                {projects.value.map((proj) => (
                  <option
                    value={String(proj.id)}
                    selected={cloneTargetId.value === String(proj.id)}
                  >
                    {proj.name}
                  </option>
                ))}
              ...
              <div class="mt-5 flex justify-end gap-2">
                ...
                <button
                  class="bg-brand hover:bg-brand-hover hover:shadow-glow rounded-md px-4 py-2 text-sm font-medium text-white transition-all"
                  onClick={doClone}
                >
                  Clone
                </button>
