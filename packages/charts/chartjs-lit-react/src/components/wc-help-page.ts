...
const SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "The platform is built with lark-react (routing shell), Lit web components (UI), TailwindCSS, Monaco Editor, chart.js, and GSAP animations. It supports dark mode and responsive layouts.",
    ],
  },
];
...
export class WcHelpPage extends WcElement {
  ...
  @state() private activeKey =
    SECTIONS.find((s) => s.key === this.sectionParam)?.key || SECTIONS[0].key;

  private selectSection(key: string): void {
    this.activeKey = key;
    this.dispatchEvent(
      new CustomEvent("nav-request", {
        detail: `/help?section=${key}`,
        bubbles: true,
        composed: true,
      }),
    );
  ...
  protected override firstUpdated(): void {
    animateIn(this, "[data-anim]", { y: 14, stagger: 0.05 });
  }
