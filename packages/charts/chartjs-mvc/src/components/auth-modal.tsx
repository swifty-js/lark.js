import { useSignal, useRef, useSignalEffect } from "@lark.js/mvc";
import { raw } from "@lark.js/mvc/jsx-runtime";
import { loginApi, registerApi } from "@/lib/api";
import { useAuthStore } from "@/lib/auth-store";
import { showAuthModal, closeAuthModal } from "@/lib/ui";
...
export default function AuthModal() {
  ...
  useSignalEffect(() => {
    if (open && !popAnimated.current && dialogRef.current) {
      popAnimated.current = true;
      animatePop(dialogRef.current);
    }
    if (!open) popAnimated.current = false;
  });
    ...
    <div>
      ...
      <div class="group border-border focus-within:border-brand focus-within:ring-brand/20 bg-surface-alt/60 relative flex items-center rounded-xl border transition-all focus-within:ring-3">
        ...
        <input
          ...
          onKeyDown={(e: Event) => {
            if ((e as KeyboardEvent).key === "Enter" && onEnter) onEnter();
          }}
        />
      </div>
    </div>
