...
const SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "The platform is built with lark-mvc, TailwindCSS, Monaco Editor, chart.js, and GSAP animations. It supports dark mode and responsive layouts.",
    ],
  },
];
...
export default function HelpPage() {
  ...
  const sectionParam = router.searchParams.value.get("section") || "start";
  const activeKey = useSignal(SECTIONS.find((s) => s.key === sectionParam)?.key || SECTIONS[0].key);
  ...
  const current = SECTIONS.find((s) => s.key === activeKey.value) || SECTIONS[0];

  return (
    <div ref={rootRef} class="mx-auto max-w-5xl px-6 py-10">
      <div class="mb-8">
      ...
      <div class="flex gap-10">
          ...
          <nav class="sticky top-20 space-y-1">
            {SECTIONS.map((item) => (
              <button
                ...
                onClick={() => {
                  activeKey.value = item.key;
                  router.navigate(`/help?section=${item.key}`);
                }}
              >
                {raw(
                  `<span class="inline-flex shrink-0 opacity-70">${icon(item.icon as never, 14)}</span>`,
                )}
                {item.title}
              </button>
            ))}
        ...
        <article class="min-w-0 flex-1">
          <h2 class="text-text-primary animate-slide-up text-xl font-semibold">{current.title}</h2>

          <div class="mt-5 space-y-4">
            {current.paragraphs.map((para) => (
              <p class="text-text-secondary animate-slide-up text-sm leading-relaxed">{para}</p>
            ))}
          </div>

          {current.code && (
            <div class="border-border animate-scale-in mt-6 overflow-hidden rounded-xl border">
              <div class="border-border bg-surface-alt flex items-center gap-2 border-b px-4 py-2">
                {raw(`<span class="text-brand/70 inline-flex">${icon("code", 13)}</span>`)}
                <span class="text-text-secondary text-xs font-medium">Example</span>
              </div>
              <pre class="bg-code-bg text-text-primary overflow-x-auto p-4 text-xs leading-relaxed">
