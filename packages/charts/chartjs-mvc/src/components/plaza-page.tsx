import { useSignal, useRef, useEffect, useSignalEffect } from "@lark.js/mvc";
...
export default function PlazaPage() {
  ...
  useEffect(() => {
    homeChartsApi()
      .then((res) => {
        ...
        loading.value = false;
        queueMicrotask(() => {
          if (rootRef.current) animateIn(rootRef.current, "[data-anim]");
        });
      })
      .catch(() => {
        loading.value = false;
  ...
  const applyFilter = (type: string) => {
    ...
    queueMicrotask(() => {
      if (rootRef.current) animateIn(rootRef.current, "[data-anim-card]", { y: 24, stagger: 0.03 });
    });
  };
  ...
  return (
    <div ref={rootRef} class="relative mx-auto max-w-7xl px-6 py-8">
      <div class="mb-8">
        <div class="flex items-end justify-between">
          <div>
            <div class="mb-2 flex items-center gap-2">
              {raw(`<span class="text-brand inline-flex">${icon("sparkles", 16)}</span>`)}
        ...
        <div class="bg-surface/75 border-border sticky top-14 z-20 -mx-6 mb-8 border-y px-6 py-3 backdrop-blur-xl backdrop-saturate-150">
          <div class="flex flex-wrap items-center gap-2">
            ...
            <div class="border-border ml-auto flex items-center gap-2 rounded-full border px-3 py-1">
              {raw(`<span class="text-text-tertiary inline-flex">${icon("search", 13)}</span>`)}
              <input
                type="text"
        ...
        <>
          ...
          {!loading.value && groups.value.length === 0 && (
            <div class="text-text-secondary flex h-64 flex-col items-center justify-center">
              {raw(`<span class="text-text-tertiary/40 inline-flex">${icon("search", 40)}</span>`)}
