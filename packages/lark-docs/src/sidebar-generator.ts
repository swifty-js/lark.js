 ...
 *
 * Groups routes by their first sub-directory under the prefix, sorts each
 * group with the "all or nothing" sidebar_position rule (see
 * ./utils/route-sorting), and produces a SidebarItem[] tree.
 */
 ...
 *
 * Grouping rules:
 * 1. Routes one segment below the prefix become top-level items; routes
 *    deeper than that are grouped under their first path segment. The tree
 *    is therefore at most two levels deep regardless of directory nesting.
 * 2. Each group is sorted by `sortDocsRoutes` — by sidebarPosition only when
 *    EVERY route in that group has one, otherwise by filename.
 * 3. `index.md` maps to the prefix path itself, so it lands in the top-level
 *    group rather than a nested one.
 * 4. Virtual index routes (`isDirectoryIndex`) are excluded to avoid
 *    duplicating the first page of a directory.
 */
