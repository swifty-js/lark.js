/**
 * State banner — proves State reactivity reaches story frames.
 *
 * The view never receives props: the story writes to the `State` singleton
 * (via `larkRender({ state })`) and the template's `State.get(...)` reads
 * subscribe this view — the write re-renders it automatically. No
 * observeState declaration, no digest call.
 */
import { defineView, jsxTemplate, State } from "@lark.js/mvc";
import styles from "./state-banner.module.css";

/** State keys this view reads. Namespaced to avoid clashes across stories. */
